...
class AviationStackClient:
            ...
            try:
                with httpx.Client(timeout=self._settings.timeout_seconds) as client:
                    ...
                    if response.status_code == 429:  # Rate limit
                        self._logger.warning("Rate limit exceeded, retrying with backoff")
                    ...
                    data = response.json()
                    self._logger.info(f"API request successful, received {len(data.get('data', []))} flights")
    ...
    def _parse_flight_record(self, flight_data: dict) -> FlightRecord:
        ...
        status_category = self._determine_delay_category(
            flight_status, 
            flight_data.get('arrival', {}).get('delay', 0)
        )
    ...
    def _determine_delay_category(self, flight_status: str, delay_minutes: Optional[int]) -> DelayCategory:
