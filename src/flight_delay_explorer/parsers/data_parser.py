...
from ..models import DelayCategory, FlightRecord
...
class FlightDataParser:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        """
        self._logger = logger or setup_logger("flight_data_parser", level=logging.INFO)
    ...
    def parse_file(self, file_path: str) -> list[FlightRecord]:
        ...
        try:
            with Path(file_path_obj).open(encoding="utf-8") as f:
                data = json.load(f)
        ...
        self._logger.info(
            f"Successfully parsed {len(records)} flight records from {file_path}"
        )
        return records
    ...
    def _parse_json_data(self, data: dict[str, Any]) -> list[FlightRecord]:
        ...
        self._logger.debug(
            f"Parsed {len(records)} valid records out of {len(flights_data)} total"
        )
        return records
    ...
    def _normalize_date_format(self, date_str: str) -> str:
        ...
        if "-" in date_str:
            parts = date_str.split("-")
            if len(parts) == 3 and len(parts[2]) == 4:  # MM-DD-YYYY or DD-MM-YYYY
    ...
    def _determine_delay_category(
        self, flight_status: str, delay_minutes: int
    ) -> DelayCategory:
