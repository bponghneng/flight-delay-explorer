...
from .models import QueryParams, FlightRecord, DelayCategory
from .api.client import AviationStackClient
from .utils.logging import setup_logger


app = typer.Typer(help="Flight Delay Explorer - Process airline on-time performance data")
console = Console()
...
def fetch(
    ...
    logger = setup_logger(
        "flight_delay_cli",
        level=log_level_map.get(log_level.upper(), 20)
    )
    ...
    try:
        ...
        logger.error(error_msg)
        console.print(f"[red]Error:[/red] {error_msg}")
        console.print("[yellow]Hint:[/yellow] Make sure FLIGHT_ACCESS_KEY environment variable is set")
        raise typer.Exit(1)
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            console.print("[yellow]Hint:[/yellow] Try again later or upgrade your API plan")
        else:
            console.print(f"[red]Error:[/red] {error_msg}")
...
def _generate_summary_stats(records: List[FlightRecord]) -> dict:
    ...
    delayed = sum(1 for r in records if r.flight_status in [
        DelayCategory.MINOR_DELAY, DelayCategory.MAJOR_DELAY, DelayCategory.SEVERE_DELAY
    ])
    ...
    avg_delay = sum(r.arrival_delay for r in delay_records) / len(delay_records) if delay_records else 0
