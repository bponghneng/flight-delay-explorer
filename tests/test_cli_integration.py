...
from flight_delay_explorer.models import FlightRecord, DelayCategory
...
class TestCLIIntegration:
    ...
    def test_fetch_command_api_integration(self, mock_setup_logger, mock_settings_class, mock_client_class, sample_api_response):
    ...
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings')
    def test_fetch_command_api_authentication_failure(self, mock_settings_class, mock_client_class):
        ...
        assert result.exit_code != 0
        assert "HTTP" in result.stdout or "401" in result.stdout or "Unauthorized" in result.stdout
    
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings')
    def test_fetch_command_network_connectivity_issues(self, mock_settings_class, mock_client_class):
    ...
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings')
    def test_fetch_command_rate_limiting_responses(self, mock_settings_class, mock_client_class):
    ...
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings')
    def test_fetch_command_malformed_api_responses(self, mock_settings_class, mock_client_class):
    ...
    def test_fetch_command_comprehensive_logging(self, mock_setup_logger):
        ...
        with patch('flight_delay_explorer.cli.AviationStackClient') as mock_client_class:
            with patch('flight_delay_explorer.cli.Settings') as mock_settings_class:
                with patch.dict(os.environ, {'FLIGHT_ACCESS_KEY': 'test-key'}):
    ...
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings')
    def test_fetch_command_display_formatting(self, mock_settings_class, mock_client_class):
    ...
    @patch('flight_delay_explorer.cli.AviationStackClient')
    @patch('flight_delay_explorer.cli.Settings') 
    def test_fetch_command_pagination_large_results(self, mock_settings_class, mock_client_class):
        ...
        for i in range(50):  # Create 50 records
            large_record_set.append(
                FlightRecord(
                    ...
                    flight_status=DelayCategory.ON_TIME if i % 2 == 0 else DelayCategory.MINOR_DELAY,
                    origin_icao="KJFK"
                )
            )
    ...
    def test_fetch_command_configuration_missing_api_key(self):
        ...
        assert result.exit_code != 0
        assert "required" in result.stdout.lower() or "api" in result.stdout.lower() or "key" in result.stdout.lower()