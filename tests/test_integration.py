...
class TestEndToEndIntegration:
    ...
    def test_data_parser_integration_with_real_structure(
        self, end_to_end_mock_response
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            on_time_record = next(
                r for r in records if r.flight_status == DelayCategory.ON_TIME
            )
            assert on_time_record.arrival_delay == 15

            cancelled_record = next(
                r for r in records if r.flight_status == DelayCategory.CANCELLED
            )
            assert cancelled_record.arrival_delay == 0

            severe_delay_record = next(
                r for r in records if r.flight_status == DelayCategory.SEVERE_DELAY
            )
